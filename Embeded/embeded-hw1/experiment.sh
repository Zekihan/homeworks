...
cacheSizes=(8 16 32 64)
blockSizes=(8 16 32 64)
associativity=(4 16 32)
for i in ${cacheSizes[@]}
do
    for j in ${blockSizes[@]}
    do
...
namespace DL1
{
    // 1st level data cache: $i kB, $j B lines, $k-way associative
    const UINT32 cacheSize = $i*KILO;
    const UINT32 lineSize = $j;
    const UINT32 associativity = $k;
    const CACHE_ALLOC::STORE_ALLOCATION allocation = CACHE_ALLOC::STORE_NO_ALLOCATE;
...
" > myfile.cpp
            cp myfile.cpp pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/allcache.cpp
            cd pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/
            make allcache.test > /dev/null

            cd ../../../../
            gcc transpose.c -o pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/out > /dev/null

            for dim in ${dimensions[@]}
            do
                echo --------------------------------------------------------------
                echo "results of $i $j $k with $dim size for original" 
                echo --------------------------------------------------------------
                ./pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/out $dim 1
                pin-3.13-98189-g60a6ef199-gcc-linux/pin -t pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/obj-intel64/allcache.so -- ./pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/out $dim 1
                echo --------------------------------------------------------------
                echo "results of $i $j $k with $dim size for modified" 
                echo --------------------------------------------------------------
                ./pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/out $dim 0
                pin-3.13-98189-g60a6ef199-gcc-linux/pin -t pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/obj-intel64/allcache.so -- ./pin-3.13-98189-g60a6ef199-gcc-linux/source/tools/Memory/out $dim 0
                echo --------------------------------------------------------------
            done
        done
    done
done
rm -r myfile.cpp > /dev/null
